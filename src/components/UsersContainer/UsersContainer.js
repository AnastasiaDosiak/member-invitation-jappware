...
export default class UsersContainer extends PureComponent {
  ...
  changeUserNames = (users, firstName, lastName, id) => users.map((user) => {
    if (user.id === id) {
  ...
  handleEdit = (firstName, lastName, id) => {
    this.setState({
      allUsers:
        this.changeUserNames(this.state.allUsers, firstName, lastName, id),
      invitedUsers:
        this.changeUserNames(this.state.invitedUsers, firstName, lastName, id)
    })
}
